...
NoCDCracker::GameExecutableStatus NoCDCracker::getGameExecutableStatus(const ByteBuffer * gameExecutableBuffer) {
	...
	else if(Utilities::areStringsEqual(gameExecutableSHA1, ATOMIC_EDITION_SPECIAL_EDITION_GAME_EXECTUABLE_UNCRACKED_SHA1)) {
		gameExecutableStatus |= GameExecutableStatus::AtomicEdition | GameExecutableStatus::SpecialEdition;
	}
	else if(Utilities::areStringsEqual(gameExecutableSHA1, ATOMIC_EDITION_SPECIAL_EDITION_GAME_EXECTUABLE_CRACKED_SHA1)) {
